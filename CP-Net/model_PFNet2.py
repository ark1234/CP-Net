...
class Latentfeature(nn.Module):
    def __init__(self,num_scales,each_scales_size,point_scales_list):
        ...
        #                            self.bn1_1,
        #                            nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(64, 256, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(64, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv9 = nn.Sequential(nn.Conv1d(128, 512, kernel_size=1, bias=False),
                                   self.bn9,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv10 = nn.Sequential(nn.Conv1d(512, 1024, kernel_size=1, bias=False),
                                   self.bn10,
                                   nn.LeakyReLU(negative_slope=0.2))
...
#new
        ...
        x1 = self.conv7(x1)
        x1 = F.adaptive_max_pool1d(x1, 1).view(batch_size, -1)
        x2 = self.conv8(x2)
        x2 = F.adaptive_max_pool1d(x2, 1).view(batch_size, -1)
        x3 = self.conv9(x3)
        x3 = F.adaptive_max_pool1d(x3, 1).view(batch_size, -1)
        x4 = self.conv10(x4)
        x4 = F.adaptive_max_pool1d(x4, 1).view(batch_size, -1)
        x = torch.cat((x1, x2, x3, x4), 1)
        latentfeature = F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.2)
