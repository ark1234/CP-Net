...
class Decoder(nn.Module):
    def __init__(self, num_coarse=256, num_dense=512):
        ...
        self.grids = torch.Tensor(grids).view(2, -1)  # (2, 2, 1) -> (2, 2)
    ...
    def forward(self, x):
        ...
        repeated_centers = y_coarse.unsqueeze(3).repeat(1, 1, 1, 2).view(b, 3, -1)  # (B, 3, 2x1024)
        # repeated_v = v.unsqueeze(2).repeat(1, 1, 16 * self.num_coarse)               # (B, 1024, 16x1024)
        repeated_v = v.unsqueeze(2).repeat(1, 1, 2 * self.num_coarse)               # (B, 1024, 2x1024)
