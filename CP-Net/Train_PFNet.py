...
# from model_PFNet2 import _netlocalD,_netG
...
parser.add_argument('--manualSeed', type=int, help='manual seed', default=611)
...
dset = shapenet_part_loader.PartDataset( root='/home/ark/local/PF-Net-Point-Fractal-Network-datasets/dataset/shapenet_part/shapenetcore_partanno_segmentation_benchmark_v0/',classification=True, class_choice=None, npoints=opt.pnum, split='train')
assert dset
...
if opt.D_choose == 1:
    for epoch in range(resume_epoch,opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # input_cropped1= input_cropped1.half()
            # input_cropped2= input_cropped2.half()
            # input_cropped3= input_cropped3.half()
            ...
            CD_LOSS_1 = opt.wtl2 * criterion_PointLoss(fake_center1,real_center_key1)
            # CD_LOSS_2 = criterion_PointLoss(fake_center2,real_center_key2)
            CD_LOSS_1_alpha1 = opt.wtl2 * alpha1*criterion_PointLoss(fake_center1,real_center_key1)
            CD_LOSS_2_alpha2 = opt.wtl2 * alpha2*criterion_PointLoss(fake_center2,real_center_key2)
...
#new
            ...
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f / %.4f/ %.4f/ %.4f / %.4f'
                  % (epoch, opt.niter, i, len(dataloader), 
                     errD.data, errG_D.data, errG, CD_LOSS_1,CD_LOSS_1_alpha1,CD_LOSS_2_alpha2))


            f=open('/home/ark/local/PF-Net-Point-Fractal-Network-datasets/Trained_Model_try602_Settled_Seed_Pytorch120_new_split_V2/loss_PFNet.txt','a')
            # f.write('\n'+'[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f / %.4f / %.4f /%.4f'
            #       % (epoch, opt.niter, i, len(dataloader), 
            #          errD.data, errG_D.data,errG_l2,errG,CD_LOSS))
            f.write('\n'+'[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f / %.4f/ %.4f/ %.4f / %.4f'
                  % (epoch, opt.niter, i, len(dataloader), 
                     errD.data, errG_D.data, errG, CD_LOSS_1,CD_LOSS_1_alpha1,CD_LOSS_2_alpha2))
        ...
        if epoch% 10 == 0:   
            torch.save({'epoch':epoch+1,
                        ...
                        '/home/ark/local/PF-Net-Point-Fractal-Network-datasets/Trained_Model_try602_Settled_Seed_Pytorch120_new_split_V2/point_netG'+str(epoch)+'.pth' )
            torch.save({'epoch':epoch+1,
                        'state_dict':point_netD.state_dict()},
                        '/home/ark/local/PF-Net-Point-Fractal-Network-datasets/Trained_Model_try602_Settled_Seed_Pytorch120_new_split_V2/point_netD'+str(epoch)+'.pth' )  
    ...
    for epoch in range(resume_epoch,opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            print('[%d/%d][%d/%d] Loss_G: %.4f / %.4f '
                  % (epoch, opt.niter, i, len(dataloader), 
                      errG_l2,CD_LOSS))
            f=open('/home/ark/local/PF-Net-Point-Fractal-Network-datasets/Checkpoint/loss_PFNet.txt','a')
        ...
        if epoch% 10 == 0:   
            torch.save({'epoch':epoch+1,
                        'state_dict':point_netG.state_dict()},
                        '/home/ark/local/PF-Net-Point-Fractal-Network-datasets/Checkpoint/point_netG'+str(epoch)+'.pth' )
